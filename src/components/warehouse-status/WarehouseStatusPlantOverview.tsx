...
const WarehouseStatusPlantOverview: React.FC<
  WarehouseStatusPlantOverviewProps
> = ({ data }) => {
  ...
  const calculatePlantStats = (plantValue: string) => {
    ...
    const checkInCount = plantData.filter(
      (item) => parseInt(item.whStatus) >= 5 && parseInt(item.whStatus) <= 9,
    ).length;
  ...
  return (
    <Row gutter={[16, 16]}>
      {PLANT_OPTIONS.map((plant) => {
        ...
        return (
          <Col key={plant.value} xs={24} sm={24} md={6} lg={6}>
            <Card
              size="small"
              style={{
                background: "#fafafa",
